...
class Table(object):
    ...
    def draw(self, screen):
        ...
        if (self.cue.vel == (0,0)):
            pygame.draw.line(screen, 'Red', self.cue.pos, pygame.mouse.get_pos())
    ...
    def move_balls(self, screen, visible, time):
        for ball in self.table_balls:
            if visible:
                ball.draw(screen)
            if (ball.vel[0] != 0 or ball.vel[1] != 0):
                ball.physics_process(cg.DELTA, time)
            
            for other_ball in self.table_balls:
                if other_ball != ball:
                        ball.check_ball_collision(other_ball)
            
            self.handle_pocket_collision(ball)
            self.handle_bumper_collision(ball)

    def handle_bumper_collision(self, ball):
        ...
        ball_high = ball.pos[1] < self.side_width + ball.radius
        ball_moving_up = ball.vel[1] < 0
        ball_low = ball.pos[1] > cg.HEIGHT - self.side_width - ball.radius
        ...
        ball_past_bumper = ball.pos[0] < self.side_width + ball.radius or ball.pos[0] > cg.WIDTH - self.side_width - ball.radius
    ...
    def handle_pocket_collision(self, ball):
